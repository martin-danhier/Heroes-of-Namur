...
def get_AI_orders(players, map, database, player):
    ...
    for hero in players[player]:
        ...
        if hero_type == 'barbarian':
            actions.append(process_barbarian(
                players, map, database, actions, player, hero))

        elif hero_type == 'healer':
            actions.append(process_healer(
                players, map, database, actions,  player, hero))

        elif hero_type == 'mage':
            actions.append(process_mage(
                players, map, database, actions, player, hero))

        elif hero_type == 'rogue':
            actions.append(process_rogue(
                players, map, database, actions, player, hero))
    # Generate a command string for those actions
    return generate_command_string(actions)

# === CLASS FUNCTIONS ===
# Check if a hero can use its abilities
...
def process_barbarian(players, map, database, orders, player, hero):
    ...
    implementation: Guillaume Nizet (v.1 05/05/2019)
    """
    ...
    if players[player][hero]['level'] >= '3':
        ...
        energise_range = database['barbarian'][str(
            players[player][hero]['level'])]['abilities'][0]['radius']
        stun_range = database['barbarian'][str(
    ...
    if players[player][hero]['level'] == '5' or len(players['creatures']) == 0:
        # Rush phase: try to control citadel
        return rush_citadel(players, map, database, orders, player, hero)
    else:
        # Farm creatures to earn xp and levels
...
def process_healer(players, map, database, orders, player, hero):
    ...
    Version
    -------
    specification: Martin Danhier (v.1 01/05/19)
    implementation: Guillaume Nizet (v.1 05/05/19)
    """
        ...
        for checked_hero in players[checked_player]:
            ...
            if checked_player == player:
                ...
                if checked_hero != hero:
                    ...
                    max_hp = database[players[checked_player][checked_hero]['type']
                                      ][players[checked_player][checked_hero]['level']]['hp']
                    radius = math.floor(
                        3 + 1/3 * get_distance(players[player][hero]['coords'], coords))
                    ...
                    data_allies.append(
                        (coords, hp, max_hp, max_hp/hp + nb_enemies_in_radius/2))
            ...
            for ally in data_allies:
                if ally[3] == max(danger_amounts):
                    ...
                    if math.floor(get_distance(ally[0], players[player][hero]['coords'])) <= database['healer'][players[player][hero]['level']]['abilities'][0]['radius']:
                        ...
                        # Go towards him
                        order = find_path(
                            players, map, orders, players[player][hero]['coords'], ally[0], False)
                        order['hero'] = hero
                        return order
...
def process_mage(players, map, database, orders, player, hero):
    ...
    # Level 2 or above
    if hero_lvl >= '2':
        fulgura_range = database['mage'][str(
            players[player][hero]['level'])]['abilities'][0]['radius']
        ...
        for player_checked in players:
            if player_checked != player:
                for hero_checked in players[player_checked]:
                    distance = math.floor(get_distance(
                        players[player][hero]['coords'], players[player_checked][hero_checked]['coords']))
...
def process_rogue(players, map, database, orders, player, hero):
    ...
    if hero_lvl >= '3':
        ...
        # distance of nearest enemies == 1 <=> there are ennemies within reach
        if distance_min_enemies == 1:
            # if there is only one enemy, attack it
            if len(closest_enemies) == 1:
                coords_enemy = players[closest_enemies[0]
                                       [0]][closest_enemies[0][1]]['coords']
                x = coords_enemy[0]
                y = coords_enemy[1]
                ...
                # Target in priority the low hp creatures, then the low hp healers, then the other low hp enemies
...
def farm_creatures(players, map, database, orders, player, hero):
    ...
    orders: the orders already given to the allies (list of dict)
    player : the name of the AI player (str)
    hero : the name of the current hero. (str)
    ...
    -------
    specification: Martin Danhier (v.1 01/05/2019)
    implementation: Martin Danhier (v.1 04/05/2019)
    """
    too_close = False
            ...
            if first_iteration:
                ...
                while index < len(spur_sorted) and not sorted:
                    ...
                    if dist < spur_sorted[index][1]:
                        spur_sorted = spur_sorted[:index] + \
                            [(tile, dist)] + spur_sorted[index:]
                        sorted = True
                ...
                if not ally_on_nearest_tile:
                    ...
                    if spur_is_full:
                        ...
                        if players[player][hero]['type'] == 'rogue' and int(players[player][hero]['level']) >= 3 and players[player][hero]['cooldown'][0] == 0:
                            ...
                            order = find_path(
                                players, map, orders, players[player][hero]['coords'], nearest_tile, True)
                        order_generated = True
            else:
                # Can reach be done ?
                if players[player][hero]['type'] == 'rogue' and int(players[player][hero]['level']) >= 3 and players[player][hero]['cooldown'][0] == 0:
                    order = find_path(players, map, orders, players[player][hero]['coords'], nearest_tile,
                                      False, database['rogue'][players[player][hero]['level']]['abilities'][0]['radius'])
                else:
                    order = find_path(
...
def find_path(players, map, orders, source, target, attack_target=True, walkable_distance=1):
    """ Create an intelligent order to move/attack the target tile.
    ...
    map: data of the map (spawns, spur, size, etc...) (dict)
    orders: the orders already given to the allies (list of dict)
    source: the coords of the hero that should move (tuple)
    target: the coords of the target tile (tuple)
    attack_target: if True, attack the target tile when it is in range. if False, only move to that target tile. (bool, default True)
    walkable_distance: the maximal distance that the hero can move in a single turn (int, default 1)

    Returns
    -------
    order: a order dictionary (dict)
    ...
    if distance == 1 and attack_target:
        ...
        for x_coord in range(source[0] - walkable_distance, source[0] + walkable_distance + 1):
            for y_coord in range(source[1] - walkable_distance, source[1] + walkable_distance + 1):
                if (x_coord, y_coord) != source:
                    ...
                    source_distance = math.floor(
                        get_distance((x_coord, y_coord), source))

                    # If the checked tile is reachable
                    if source_distance <= walkable_distance:
                        if first_loop:
                            distances.append(
                                ((x_coord, y_coord), tile_distance))
                            first_loop = False
                        else:
                            index = 0
                            sorted = False
                            ...
                            while not sorted and index < len(distances):
                                if tile_distance <= distances[index][1]:
                                    distances = distances[:index] + [
                                        ((x_coord, y_coord), tile_distance)] + distances[index:]
                                    sorted = True
        ...
        distance_from_source = math.floor(
            get_distance(order['target'], source))
